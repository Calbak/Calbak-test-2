...
class AddPictureViewController: UIViewController{
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        imgPickerController.delegate = self    //인스턴스의 delegate를 self로 설정
        PHPhotoLibrary.requestAuthorization{ (state) in print (state)}
    ...
    @IBAction func BtnCamera(_ sender: Any) {
        if CameraAuth() {
            self.imgPickerController.sourceType = .camera  //imgPickerController의 sourceType을 photoLibrary로 설정
...
extension AddPictureViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    func AuthSettingOpen(AuthString: String){
        if let AppName = Bundle.main.infoDictionary!["CFBundleName"] as? String {
            ...
            let cancle = UIAlertAction(title: "취소", style: .default) { (UIAlertAction) in print("\(String(describing: UIAlertAction.title)) 클릭")
            }
